...
def make_plots(results: dict):
    ...
    median_vals = [valid[c]["median_ms"] for c in cities]
    theor_vals  = [valid[c]["theoretical_min_ms"] for c in cities]

    ax.bar(x - width/2, median_vals, width, label="Measured Median RTT", color="#457b9d")
    ax.bar(x + width/2, theor_vals,  width, label="Theoretical Min RTT",  color="#e63946")
    ...
    dist_range = np.linspace(0, max(valid[c]["distance_km"] for c in cities) * 1.05, 300)
    theor_line = (dist_range / FIBER_SPEED_KM_S) * 2 * 1000
    ax.plot(dist_range, theor_line, "k--", linewidth=1.5, label="Theoretical minimum")

    for c in cities:
        continent = valid[c]["continent"]
        color = CONTINENT_COLORS.get(continent, "#888888")
        ax.scatter(valid[c]["distance_km"], valid[c]["median_ms"], color=color, s=80, zorder=5)
        ax.annotate(c, (valid[c]["distance_km"], valid[c]["median_ms"]),
                    textcoords="offset points", xytext=(6, 4), fontsize=8)

    legend_patches = []
    for continent, color in CONTINENT_COLORS.items():
        if any(valid[c]["continent"] == continent for c in cities):
            legend_patches.append(mpatches.Patch(color=color, label=continent))
    legend_patches.append(plt.Line2D([0], [0], color="k", linestyle="--", label="Theoretical minimum"))
